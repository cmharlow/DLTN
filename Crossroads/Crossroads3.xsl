...
<<<<<<< HEAD:Crossroads/Crossroads.xsl
    <xsl:template match="text()|@*"/>
...
>>>>>>> 800456c2f0aa4b9597368076326185515bdb410d:Crossroads/Crossroads2.xsl
...
<!-- Grouped Templates -->
    ...
    <xsl:template name="originInfo">
        <xsl:for-each select="dc:date|dc:publisher">
    ...
    <xsl:template name="recordInfo">
        ...
        <recordInfo>
            ...
            <recordCreationDate><xsl:value-of select="../../header/datestamp"/></recordCreationDate>
...
>>>>>>> f0afdbfbc337c56890fa36b1c9bba19699e7fa15
    ...
    <xsl:template match="dc:source">
        <xsl:choose>
            <xsl:when test="contains(lower-case(.), 'collection') or contains(lower-case(.), 'corp') and not(dc:identifier)">
                <relatedItem type="host" displayLabel="Collection">
                    <title>
                        <titleInfo>
                            <xsl:value-of select="normalize-space(.)"/>
        ...
        <xsl:if test="not(starts-with(lower-case(normalize-space(.)), 'http://')) and dc:identifier">
            <identifier>
                <xsl:value-of select="."/>
            </identifier>
        </xsl:if>
